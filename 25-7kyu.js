...
function operate(pcSet, operation) {
   const match = operation.match(/T(\d+)(I)?/)

   const n = parseInt(match[1], 10)
   const isInversion = match[2] === "I"

   let result

   if (isInversion) {
      result = pcSet.map((pitch) => (12 - pitch + n) % 12)
   } else {
      result = pcSet.map((pitch) => (pitch + n) % 12)
   }
   return result.sort((a, b) => a - b)
}

console.log(operate([1, 2, 3], "T5I"))
